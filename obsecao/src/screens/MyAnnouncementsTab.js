...
export default class MyAnnouncementsTab extends React.Component {
    constructor(props) {
        super(props);
    ...
    loadFeed() {
        ...
        return FeedService.getMyAnnouncements()
            .then(feed => {
                if (!feed) feed = []
...
function goToNextScreen(key) {
    NavigationService.announcementManagement.navigateTo({idAnnouncement: key})
}
